...
public final class JavaSparkPi {
    ...
    public static void main(String[] args) throws Exception {
        ...
        SparkSession spark = SparkSession.builder().appName("JavaSparkFile").master("yarn").config(conf).getOrCreate();
        ...
        int count = dataSet.map(new Function<Integer, Integer>() {
            ...
            public Integer call(Integer integer) throws Exception {
                ...
                // spark's metric system 要在每个 executor 上去获取.
                Seq<Source> executorSource = SparkEnv.get().metricsSystem().getSourcesByName("executor");
                Source source = executorSource.apply(0);
                System.out.println(source instanceof ExecutorSource);
                ExecutorSource executor = (ExecutorSource) source;
                SortedMap<String, Gauge> gauges = executor.metricRegistry().getGauges();
                Object activeTasks = gauges.get("threadpool.activeTasks").getValue();
                Object completeTasks = gauges.get("threadpool.completeTasks").getValue();

                System.out.println("executorSource:::" + executorSource.size());
                System.out.println("activeTasks:::" + activeTasks);
                System.out.println("completeTasks:::" + executorSource.size());

                logger.error("executorSource:::" + executorSource.size());
                logger.error("activeTasks:::" + activeTasks);
                logger.error("completeTasks:::" + completeTasks);
        ...
        Seq<Source> executorSource = SparkEnv.get().metricsSystem().getSourcesByName("executor");
        System.out.println("driverExecutorSource:::" + executorSource.size());
